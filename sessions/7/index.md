---
title: Session 7
---

## Office Hours - Group Session

A few of you have asked about office hours. I am thinking of planning a group session for the weekend, on Sunday. If you are interested, see me after class and we can find a time that works for all of us.

## Reading discussion activity

We watched this lecture, as our reading for this week:

[Muriel Cooper \| David Reinfurt \| Talks at Google](https://youtu.be/U_PGr5d9r7Q?si=obed38qjVAS7Mx0g) – David Reinfurt

- Let's skip around the video and talk about the work shown. What do we see?
- What's one thing you learned?
- What's one thing you're curious about?
- In your opinion, what are the biggest similarities and differences between print and digital design?

Share these thoughts here: [document](https://docs.google.com/document/d/1Qh69L_0kZR0UhI0nUy3y29JznWSJC5G-7gUXooXNJd4/edit?usp=sharing)

## Lecture

We're going to talk about how flex and grid layouts work in CSS:

- [Flexbox & grid]({{ site.baseurl }}/lectures/flexbox-and-grid)

## Break

## Activity: recreating art with CSS

From beginning to end, we are going to make webpage recreations of the following two artworks:

{% include figure.html src='homage_apparition.jpg' caption='Homage to the Square: Apparition, Josef Albers' %}

{% include figure.html src='mondrian.jpg' caption='Composition with Red Blue and Yellow, Piet Mondrian' %}

## Site browsing

If we have time today, let's browse some of your favorite sites and talk about their design.

## For next class

Next class we have the midterm edit. You will sign up for a 1-on-1 time with me in this classroom on this sheet:

[Sign-up here](https://docs.google.com/document/d/1iLbFxS_tnEOFYUDVjxBtnE9Z6_la0TN1xyeP7K4tngk/edit?usp=sharing)

Outside of your meeting time, you do not need to come to class.

### Midterm Edit

For Midterms, we will have a one on one meeting. At that time, you will present a refined iteration of your current collection. Prior to this meeting, think about the following questions:

- What is your theme? Has it evolved from the initial description?
- How is the content communicating the theme? What about the design?
- How would you describe the pacing of the collection so far? Does it feel considered? Can there be a moment for a dramatic break, or an acceleration?
- Are there opportunities to establish a template or system to the design — if so, when, where, and how? Can we break the system mindfully to add interest?

You will also prepare a short written doc (one paragraph) that explains your collection’s theme so far and explains three other avenues you can explore for the remaining weeks of the semester.
